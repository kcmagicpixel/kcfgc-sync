...
export default function Posts() {
  ...
  const [editAttachmentMode, setEditAttachmentMode] = useState<AttachmentMode>("none");
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editAttachmentMode === "images") {
        ...
        if (editEmbedImage) {
          const { base64, mimeType } = await resizeImage(editEmbedImage, 1_000_000, 2000);
          const res = await apiFetch("/api/posts/images", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ data: base64, mimeType }),
                    ...
                    {editAttachmentMode === "images" && (
                      <div className="flex flex-col gap-2">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {editImageIds.map((id) => (
                              <div key={id} className="relative">
                                ...
                                <Button
                                  onPress={() =>
                                    setEditImageIds((ids) => ids.filter((i) => i !== id))
                                  }
                        ...
                        {editImageIds.length + editNewFiles.length < 4 && (
                          <div className="flex flex-col gap-1">
                            <label className="text-sm font-medium text-foreground">
                              Add Images (up to {4 - editImageIds.length - editNewFiles.length})
                            </label>
                            <input
                              type="file"
                              accept="image/*"
                              ...
                              onChange={(e) => {
                                ...
                                if (!selected) return;
                                const max = 4 - editImageIds.length - editNewFiles.length;
                          ...
                          <div className="flex flex-wrap gap-2">
                            {editNewFiles.map((file, i) => (
                              <div key={i} className="relative">
                                ...
                                <Button
                                  onPress={() =>
                                    setEditNewFiles((files) => files.filter((_, j) => j !== i))
                                  }
                                  className="absolute -top-1.5 -right-1.5 flex h-5 w-5 cursor-pointer items-center justify-center border border-destructive bg-destructive text-xs text-white"
                                >
                                  ×
